...
const EmergencyPlan = () => {
  ...
  const [newContact, setNewContact] = useState({ name: '', phone: '', relation: '', type: 'emergency' as const });
  const [newInstruction, setNewInstruction] = useState({ title: '', steps: [''] });
  ...
  const addInstruction = (e: React.FormEvent) => {
    ...
    if (newInstruction.title && newInstruction.steps[0]) {
      setInstructions([...instructions, { ...newInstruction, id: Date.now().toString() }]);
